...
uint16_t _hd44780_read(void);
void _hd44780_write(uint16_t gpios);
...
static void _hd44780_write(uint16_t gpios){
  ...
  gpio_set_mode(GLCD_DATA_BANK, GPIO_MODE_OUTPUT_50_MHZ,
		      GPIO_CNF_OUTPUT_PUSHPULL, ALL_DATA_PINS);
  ...
  gpio_set_mode(LCD_DATA_BANK, GPIO_MODE_OUTPUT_50_MHZ,
		      GPIO_CNF_OUTPUT_OPENDRAIN, ALL_DATA_PINS);
  gpio_set(LCD_DATA_BANK, ALL_DATA_PINS);
}
...
void hd44780_init(){
  ...
  hd44780_clear();
  hd44780_command(LCD_CMD_AUTO_INC_CUR);
}

void hd44780_onoff(uint8_t screen_on, uint8_t cursor_dash, uint8_t cursor_blink){
  hd44780_command(LCD_CMD_ON(screen_on, cursor_dash, blink));
}
